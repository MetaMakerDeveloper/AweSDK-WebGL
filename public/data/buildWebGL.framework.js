...
Module['XMLHttpRequest'] = function() {
  ...
  xhr._open = xhr.open;
  xhr.open = function() {
    let request_url = arguments[1]
    if(request_url && request_url.startsWith("file://")){
      this._url = request_url.substr(6)
  ...
  xhr.send = function() {
    if (xhr._url) {
      let result = FS.readFile(xhr._url);
      let path = xhr._url;
      let extension=path.substring(path.lastIndexOf(".")+1,path.length);
      let blob_type = "application/octet-stream";
      if(extension === "jpg" || extension === "jpeg"){
        ...
        blob_type = "video/mpeg";
      }else if(extension === "wav"){
        blob_type = "audio/wav";
      }else{
        console.error("not supported blob format");
      }

      let file_blob = new Blob([result], {type: blob_type});
      xhr._url = (URL || webkitURL).createObjectURL(file_blob);
      result = null;
      file_blob = null;

      var _argument = xhr._argument;
      xhr._argument = null;
      _argument[1] = xhr._url;
      
      xhr._open.apply(xhr, _argument);
      xhr._send.apply(xhr, arguments);

      xhr._onload = xhr.onload
      xhr.onload = function () {
        let request_url = xhr._url
        xhr._url = null
        ...
        xhr.onload = xhr._onload
        xhr._onload = null

        xhr.onerror = xhr._onerror
        xhr._onerror = null

        xhr.onload.apply(this, arguments);
        
        

        (URL || webkitURL).revokeObjectURL(request_url)
      }
      ...
      xhr.onerror = function () {
        ...
        xhr.onerror.apply(this, arguments);
...
function FileWriter(fileEntry) {
  ...
  this.write = function(data,append) {
    ...
    if (blob_ && append) {
      ...
      var tail = blob_.slice(position_, blob_.size);



      // Do the "write". In fact, a full overwrite of the Blob.
      // TODO: figure out if data.type should overwrite the exist blob's type.
      blob_ = new Blob([head, data, tail], {type: blob_.type});
    } else {
...
idb_.get = function(fullPath, successCallback, opt_errorCallback) {
  ...
  var range = IDBKeyRange.only(fullPath);
...
Module['SendMessageToUnity'] = function (json) {
	if (window.MyUnityInstance == null) {
		console.error('SendMessageToUnity, UnityInstance is null.');
		return;
	}
	window.MyUnityInstance.SendMessage('MessageLogicJS', 'OnReveiveMessage', json);
}

Module['SendSignalMessageToUnity'] = function (json) {
	if (window.MyUnityInstance == null) {
		console.error('SendSignalMessageToUnity, UnityInstance is null.');
		return;
	}
	window.MyUnityInstance.SendMessage('MessageLogicJS', 'OnReveiveSignalMessage', json);
}

Module["ReceiveMessage"] = function (callback) {
	if (window.MyUnityInstance == null) {
		console.error('ReceiveMessage, UnityInstance is null.');
		return;
	}
	window.MyUnityInstance.Module['MessageCallBack'] = callback;
}

Module['SendMessageToJS'] = function (json) {
	if (window.MyUnityInstance == null) {
		console.error('SendMessageToJS, UnityInstance is null.');
		return;
	}
	if(window.MyUnityInstance.Module['MessageCallBack']){
		window.MyUnityInstance.Module.MessageCallBack(json);
	}
}

Module["ReceiveSignal"] = function (callback) {
	if (window.MyUnityInstance == null) {
		console.error('ReceiveSignal, UnityInstance is null.');
		return;
	}
	window.MyUnityInstance.Module['SignalCallBack'] = callback;
}

Module["SendSignalToJS"] = function (json, payload) {
	if (window.MyUnityInstance == null) {
		console.error('SendSignalToJS, UnityInstance is null.');
		return;
	}
	if(window.MyUnityInstance.Module['SignalCallBack']) {
		window.MyUnityInstance.Module.SignalCallBack(json, payload);
	}
}
...
function _JSCallBackPlugin_CallBackToJs(json) {
 var jsonStr = UTF8ToString(json);
 window.MyUnityInstance.Module.SendMessageToJS(jsonStr);
}

function _JSCallBackPlugin_SignalToJs(signal, payload) {
 var signalStr = UTF8ToString(signal);
 var payloadStr = UTF8ToString(payload);
 window.MyUnityInstance.Module.SendSignalToJS(signalStr, payloadStr);
}
...
function _glClear(mask) {
 let color = [ 0, 0, 0, 0 ];
 const clearColor = Module["glClearColor"];
 if (clearColor != null && Array.isArray(clearColor) && clearColor.length == 4) color = clearColor;
 _glClearColor(...color);
 if (mask == 16384) {
...
var asmLibraryArg = {
 "JSCallBackPlugin_CallBackToJs": _JSCallBackPlugin_CallBackToJs,
 "JSCallBackPlugin_SignalToJs": _JSCallBackPlugin_SignalToJs,
...
var dynCall_iiiiifffii = Module["dynCall_iiiiifffii"] = createExportWrapper("dynCall_iiiiifffii");
...
var dynCall_viddddddiii = Module["dynCall_viddddddiii"] = createExportWrapper("dynCall_viddddddiii");

var dynCall_viiddddddii = Module["dynCall_viiddddddii"] = createExportWrapper("dynCall_viiddddddii");
...
var dynCall_viffffffiii = Module["dynCall_viffffffiii"] = createExportWrapper("dynCall_viffffffiii");
...
var dynCall_viiiijjiiiiiii = Module["dynCall_viiiijjiiiiiii"] = createExportWrapper("dynCall_viiiijjiiiiiii");
...
var dynCall_fifffffffi = Module["dynCall_fifffffffi"] = createExportWrapper("dynCall_fifffffffi");
...
var dynCall_viiifffffiii = Module["dynCall_viiifffffiii"] = createExportWrapper("dynCall_viiifffffiii");

var dynCall_viiifffffii = Module["dynCall_viiifffffii"] = createExportWrapper("dynCall_viiifffffii");
...
var dynCall_viiiiiiiiiiif = Module["dynCall_viiiiiiiiiiif"] = createExportWrapper("dynCall_viiiiiiiiiiif");
...
function invoke_iiiifffffii(index, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10) {
 var sp = stackSave();
 try {
  return dynCall_iiiifffffii(index, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10);
 } catch (e) {
  stackRestore(sp);
  if (e !== e + 0 && e !== "longjmp") throw e;
  _setThrew(1, 0);
